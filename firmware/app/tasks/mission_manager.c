...
static inline int32_t handle_fdir_evs(const struct conops_fsm *ctx,
                                      const struct conops_event *ev)
{
    sat_data_t *sat = ctx->user_data;
...
static int goto_nominal_mode(struct conops_fsm *ctx,
                             const struct conops_event *ev,
...
static int32_t event_mapper(const struct conops_fsm *ctx,
                            const struct conops_event *ev)
{
    sat_data_t *sat = ctx->user_data;
    ...
    case EV_TC_ENTER_HIBERNATION:
    {
        ...
        sys_log_print_event_from_module(SYS_LOG_INFO, TASK_MISSION_MANAGER_NAME,
                                        "Hibernating for ");
...
int8_t notify_event_to_mission_manager(const struct conops_event *ev)
{
    ...
    {
        sys_log_print_event_from_module(SYS_LOG_ERROR, "Event Queue",
