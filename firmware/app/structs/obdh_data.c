...
int8_t obdh_get_param(uint8_t param_id, uint32_t *buf)
{
    ...
    case OBDH_PARAM_ID_TS_LAST_CONTACT:
        *buf = sat_data_buf.obdh.data.ts_last_contact;
        break;
    case OBDH_PARAM_ID_TS_COMMISSION_TIMEOUT:
        *buf = sat_data_buf.obdh.data.ts_commission_timeout;
        break;
    case OBDH_PARAM_ID_EPS_BEACON_ON:
        *buf = sat_data_buf.obdh.data.eps_beacon_on;
        break;
    case OBDH_PARAM_ID_BATT_CRITICAL_LEVEL_MV:
        *buf = sat_data_buf.obdh.data.batt_crit_level_mv;
        break;
    case OBDH_PARAM_ID_HIBERNATION_ON:
        *buf = sat_data_buf.obdh.data.hibernation_on;
        break;
    case OBDH_PARAM_ID_MANUAL_EXPERIMENT_ON:
        *buf = sat_data_buf.obdh.data.manual_experiments;
        break;
    case OBDH_PARAM_ID_TC_QUEUE_SIZE:
        *buf = sat_data_buf.obdh.data.tc_queue_size;
        break;
    case OBDH_PARAM_ID_TS_NEXT_SCHED_TC:
        *buf = sat_data_buf.obdh.data.ts_next_sched_tc;
        break;
    case OBDH_PARAM_ID_PAYLOAD_TELEMETRY_ON:
